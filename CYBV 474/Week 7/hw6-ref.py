...
DEBATE_FILE = "../Week 6/debateRaw.txt"
...
def main():
    ...
    bloombergText = ''
    topWords = {}
    with open(DEBATE_FILE, encoding='unicode_escape') as debate:
    ...
    countsBiden = Counter(tag for word, tag in wordsBiden)
    countsAmy = Counter(tag for word, tag in wordsAmy)
    countsBloom = Counter(tag for word, tag in wordsBloom)
    countsBernie = Counter(tag for word, tag in wordsBernie)
    countsButt = Counter(tag for word, tag in wordsButt)
    countsWarren = Counter(tag for word, tag in wordsWarren)
